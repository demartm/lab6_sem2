      ...
      if(Tab[expression[i]] == operands){
        ...
        if(Tab[expression[i]] == low_prio){
          if(last == 0 || (Tab[last] > operands && Tab[last] <= open_br  || Tab[last] == equ)) {//unary minus
            ...
            while(st.next && (Tab[(int)(st.next->x)] == low_prio || Tab[(int)(st.next->x)] == high_prio)){
              char buf = 0;
          ...
          if(Tab[expression[i]] == high_prio){
            ...
            if(st.size > 0 && (Tab[(int)(st.next->x)] == high_prio)){
                char buf = 0;
                popFromStack(&st,NULL,&buf);
              if(arr_ix < max_rpn){
                arr[arr_ix] = buf;
            ...
            while(st.size > 0 && Tab[(int)(st.next->x)] != open_br){
                char buf = 0;
                popFromStack(&st,NULL,&buf);
              if(arr_ix < max_rpn){
                arr[arr_ix] = buf;
            ...
            if(st.size == 0){
              ...
              i = ex_size;
                ...
                if(Tab[expression[i]] == equ){
                   while(st.next && Tab[(int)(st.next->x)] != open_br){
                    char buf = 0;
                    popFromStack(&st,NULL,&buf);
                  if(arr_ix < max_rpn){
                    arr[arr_ix] = buf;
